...
def add_split_slide_with_image(slide, prs, page_num, section_tag, title, subtitle, bullets_data, image_path, img_caption="", mascot_tip=""):
    ...
    img_top = Inches(1.8)
    img_w = Inches(5.7)
    img_h = Inches(4.1) if mascot_tip else Inches(4.5)
...
def add_table_slide(slide, prs, page_num, section_tag, title, subtitle, headers, rows_data, col_widths, top_card_text="", mascot_text=""):
    ...
    cur_top = Inches(1.8)
    if top_card_text:
        t_card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), cur_top, Inches(11.7), Inches(0.55))
        t_card.fill.solid()
    ...
    t_height = Inches(0.4 + 0.38 * len(rows_data))
    ...
    if mascot_text:
        ...
        if os.path.exists(IMG_MASCOT):
            slide.shapes.add_picture(IMG_MASCOT, Inches(0.8), m_top, Inches(0.9), Inches(0.9))
        m_box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.9), m_top, Inches(10.6), Inches(0.9))
        m_box.fill.solid()
...
def build_complete_v3_deck():
    ...
    for i, (num, unit, title, bullets) in enumerate(kpis):
        ...
        tb = s2.shapes.add_textbox(x + Inches(0.15), Inches(1.95), Inches(2.5), Inches(3.3))
        tf = tb.text_frame
        tf.word_wrap = True
        p0 = tf.paragraphs[0]
    ...
    add_split_slide_with_image(s9, prs, 9, "PART II. 제안사 역량", "주식회사 삼원팜텍 회사 개요 및 제조 인프라",
                               ...
                               {"title": "(주)삼원팜텍 핵심 역량 지표",
                                ...
                                "bullets": [
                                    "회사명: 주식회사 삼원팜텍 (대표이사 김한호)",
                                    "소재지: 충청북도 옥천군 옥천읍 테크노밸리로 본사 및 제1공장",
                                    "주요 사업: 동물약품, 보조사료, 단미사료, 기능성 프리믹스 제조",
                                    "인증 현황: KVGMP(동물용의약품 우수제조기준), 사료 HACCP 인증",
                                    "시설 규모: 마이크로 믹싱 타워, 무균 포장 라인, 정밀 분석 연구소",
                                    "생산 역량: 월간 300 M/T 고농축 프리믹스 자동화 생산 능력 보유"
                                ]},
    ...
    add_split_slide_with_image(s11, prs, 11, "PART II. 품질 보증", "옥천 공장 품질 관리 체계 및 매 배치별 성분 분석(COA)",
                               ...
                               {"title": "삼원팜텍 5단계 무결점 품질 검증",
                                "tag": "QUALITY CONTROL",
                                "bullets": [
                                    "1단계: 수입 원료 통관 시 HPLC 유효 성분 순도 100% 전수 검사",
    ...
    add_split_slide_with_image(s12, prs, 12, "PART III. 경제성 분석", "표준 투입량 500g/ton (0.05%) 처방의 공학적 의의",
                               ...
                               {"title": "왜 500g/ton이 가장 이상적인가?",
                                "tag": "500G/TON VALUE",
                                "bullets": [
                                    "원료 분자 농축화: 일반 2kg 제품과 동일한 유효 활성 분자를 500g에 응축",
                                    "영양소 치환 제로: 불필요한 부형제 투입 방지로 조단백/에너지 손실 0%",
                                    "물류비 절감: 월 18톤 운송 -> 9톤 운송으로 운송비 및 보관비 50% 절감",
                                    "작업 편의성: 500g 알루미늄 파우치 1포 = 1톤 믹서 투입으로 작업 간소화",
                                    "환경 부하 저감: 포장재 및 부형제 폐기물 75% 감소"
                                ]},
    ...
    add_split_slide_with_image(s13, prs, 13, "PART III. 경제성 분석", "사료 톤당 첨가 비용 3,000원과 가격 경쟁력",
                               ...
                               {"title": "사료 톤당 첨가제 비용 비교",
                                "tag": "COST SAVINGS",
                                "bullets": [
                                    "시중 A사 일반 처방 (1.5kg/t x 4,000원): 톤당 6,000원",
                                    "시중 B사 프리미엄 처방 (1.0kg/t x 8,000원): 톤당 8,000원",
                                    "삼원팜텍 고농축 프리믹스 (0.5kg/t x 6,000원): 톤당 3,000원",
                                    "★ 톤당 3,000원 ~ 5,000원 순수 원가 절감 효과 발생",
                                    "한일사료 임가공 배합 라인 전용 특화 단가 정책 적용"
                                ]},
    ...
    for i, (title, spec, effect, col) in enumerate(sol_cards):
        ...
        tb = s15.shapes.add_textbox(x + Inches(0.15), y + Inches(0.15), Inches(2.5), Inches(1.95))
        tf = tb.text_frame
        tf.word_wrap = True
        p0 = tf.paragraphs[0]
        p0.text = title
    ...
    add_split_slide_with_image(s18, prs, 18, "PART IV. 솔루션 상세", "04. HZM 고활성 효모균 & 05. 3종 복합생균제",
                               ...
                               {"title": "반추위 및 장내 마이크로바이옴 정상화",
                                ...
                                "bullets": [
                                    ...
                                    "섬유소 분해균(Fibrobacter) 증식 촉진 -> 휘발성지방산(VFA) +18% 증가",
                                    "HZM 3종 생균제: 유산균(L. acidophilus) + 고초균(B. subtilis) + 낙산균(C. butyricum)",
                                    "마이크로 캡슐화로 위산 및 담즙산 통과 후 소장/대장 정착률 95%",
    ...
    add_split_slide_with_image(s21, prs, 21, "PART V. 공정 적합성", "한일사료 라인 혼화도 검증: 변이계수(CV) 3.8% 달성",
                               ...
                               {"title": "마이크로 도징 혼화도 공학 시험 결과",
                                ...
                                "bullets": [
                                    "시험 조건: 한일사료 Twin-Shaft 고속 패들 믹서 (1.0 M/T 배치)",
                                    "혼합 시간: 90초 ~ 120초 표준 가동 조건 시험",
                                    "샘플링: 믹서 상/중/하/전/후 10개 포인트 정밀 채취",
                                    "시험 결과: 변이계수(CV) 3.8% 달성 (공정 기준 CV < 5.0% 완벽 충족)",
                                    "공학적 원리: 유효 원료의 미세 분말화 및 정전기 방지 담체 코팅"
                                ]},
    ...
    add_split_slide_with_image(s22, prs, 22, "PART V. 공정 적합성", "펠렛 가공열(85~110℃) 내열성 및 생존율 95% 보증",
                               ...
                               {"title": "펠렛 및 익스팬더 가공열 생존 메커니즘",
                                ...
                                "bullets": [
                                    ...
                                    "코팅 기술: 융점 85℃ 지질 매트릭스 + 열차단 미네랄 외벽 2중 보호",
                                    "효소제 생존율: 90℃ 통과 후 상대 잔존 역가 94.8% 유지",
                                    "생균/효모 생존율: 펠렛 압출 통과 후 생균수 95.2% 생존 확인",
    ...
    add_split_slide_with_image(s23, prs, 23, "PART V. 공정 적합성", "고결 방지(Anti-Caking) 및 자유 유동성 설계",
                               ...
                               {"title": "물리적 안정성 및 보관 편의성",
                                ...
                                "bullets": [
                                    ...
                                    "마이크로 빈 투입 시 관로 막힘이나 브릿지(Bridging) 현상 제로",
                                    "안티케이킹 처방: 식품용 친환경 침강 실리카 1.5% 배합으로 수분 흡착 차단",
                                    "고차단 알루미늄 포장: 3중 라미네이팅 스탠딩 파우치로 습기 100% 차단",
                                    "유통기한: 상온 12개월 품질 보증 (로트별 유통기한 명확 인자)"
                                ]},
                               IMG_POUCH, "삼원팜텍 500g 전용 고차단성 알루미늄 스탠딩 파우치 패키지 디자인",
    ...
    phases = [
        ("Phase 1: 시험 배합 & 공정 검증", "1~2주차 (준비 단계)", [
            "한일사료 라인 500g 투입 프로토콜 설정",
            "Twin-Shaft 믹서 혼화도(CV) 현장 측정",
    ...
    p = tf30.add_paragraph()
    r = p.add_run()
    r.text = "공식 제조 및 공급원: 주식회사 삼원팜텍\n대표이사 김한호 (직인생략)\n본사: 충북 옥천군 옥천읍 테크노밸리로 88\n전국 총판 및 고객상담: 043-731-8800  |  support@samwonpharm.co.kr"
    r.font.name = "Malgun Gothic"
    r.font.size = Pt(10)
    r.font.bold = True
    r.font.color.rgb = C_EMERALD
