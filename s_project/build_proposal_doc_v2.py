...
def create_document():
    ...
    add_figure(doc, IMG_FACTORY, "(주)삼원팜텍 충북 옥천테크노밸리 본사 및 첨단 R&D 제조 시설 전경", width=Inches(6.2))
    ...
    # Mascot banner
    add_figure(doc, IMG_MASCOT, "삼원팜텍 스마트 바이오 닥터 마스코트 (친환경 축산과 과학적 처방의 동반자)", width=Inches(3.2))
    ...
    p.add_run(
        ...
        "이러한 환경에서 S-NACF(서울축협)는 한일사료에서 임가공 생산하는 월 18,000톤의 배합사료 라인에 대해, "
    ...
    p.add_run(
        ...
        "• S-NACF 월간 총 투입비용: 5,400만원 (월 18,000톤 생산 기준, 총 소요량 9,000 kg)\n"
        "시중 일반 처방(톤당 6,000~9,000원) 대비 톤당 3,000원 이상의 제조 원가를 직접 절감하여 월 최대 1억 800만원(연간 13억원)의 절감 효과를 창출합니다."
    )
    add_figure(doc, CHART_COST, "사료 톤당 첨가제 투입비용 비교 분석 차트 (삼원팜텍 단 3,000원 실현)", width=Inches(5.8))

    # ================= CHAPTER 4 =================
    doc.add_page_break()
    add_styled_heading(doc, "제 4 장. 삼원팜텍 8대 핵심 솔루션 기술 상세", level=1)
    ...
    # ================= CHAPTER 6 =================
    doc.add_page_break()
    add_styled_heading(doc, "제 6 장. 기대 효과 및 실행 로드맵", level=1)
    
    IMG_HANWOO_CERT = os.path.join(ASSETS_DIR, "korean_cattle_beef_quality.jpg")
    add_figure(doc, IMG_HANWOO_CERT, "한우 1++ 등급 최고급 마블링 공식 인증 및 비육우 사육 현장", width=Inches(5.6))
    add_figure(doc, IMG_HANWOO, "한우 비육우 1++등급 출현율 12% 향상 및 ADFI 10% 개선 스마트 우사", width=Inches(5.6))
    add_figure(doc, IMG_DAIRY, "젖소 하절기 고온 스트레스 극복 및 산유 피크 유지 첨단 낙농 축사", width=Inches(5.6))
    add_figure(doc, IMG_PIGLET, "이유자돈 설사 폐사율 제로화 및 출하일령 7일 단축 건강한 자돈 돈사", width=Inches(5.6))
    add_figure(doc, CHART_ROI, "축종별 정량적 개선 효과 및 투자 대비 농가 실익 환원(1:7.2) 인포그래픽", width=Inches(5.6))
    ...
    print(f"Successfully saved enhanced docx to {output_path}")
